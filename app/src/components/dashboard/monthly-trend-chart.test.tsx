...
describe("MonthlyTrendChart", () => {
	it("renders the card title — en", () => {
		renderWithI18n(
			"en",
			"EUR",
			<MonthlyTrendChart totals={totals(3)} monthlyAmount={monthly} />,
		);

		expect(screen.getByText("Monthly totals")).toBeTruthy();
	});

	it("shows the placeholder when fewer than 2 cycles have data — en", () => {
		// Only one cycle has any entries → no trend yet.
		renderWithI18n(
			"en",
			"EUR",
			<MonthlyTrendChart totals={totals(1)} monthlyAmount={monthly} />,
		);
	...
	it("renders the chart card without throwing when ≥2 cycles have data", () => {
		...
		expect(() =>
			renderWithI18n(
				"en",
				"EUR",
				<MonthlyTrendChart totals={totals(4)} monthlyAmount={monthly} />,
			),
		).not.toThrow();
		expect(screen.getByText("Monthly totals")).toBeTruthy();
